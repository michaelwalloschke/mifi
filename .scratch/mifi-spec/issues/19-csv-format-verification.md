...
4. **Consorsbank** (added by [History backfill cut](20-history-backfill-cut.md) — gap-bridge import is now planned, not contingent): export Umsätze CSV for Giro + Tagesgeld (needs to cover 2025-09 → now for Tagesgeld); capture encoding, separator, decimal format, header set, date format, and whether a booked/pending marker exists.
