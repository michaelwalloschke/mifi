...
- Refresh/sync UX: how often to fetch, background vs manual, how TAN challenges surface. Sources fixed (FinTS + scalable-cli + manual CSV imports); mechanics now fixed too ([Backend stack decision](issues/06-backend-stack.md): Rust core supervises long-running Python sidecar, TAN as challenge/response events over stdio). Only the domain model still gates this.
